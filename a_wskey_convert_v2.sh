#!/bin/bash
# new Env('BBK-wskey转换V2');
# cron 0 0/12 * * * a_wskey_convert_v2.sh
# export JD_LOG_XYZ_TOKEN="从机器人获取的token"
# export BBK_V2_WSKEY_ENC_KEY="" #加密/解密key，启用加密的时候需要用到
# export BBK_V2_WSKEY_DISABLE_FAIL="true" #自动禁用失效wskey
pwd
_ftype=""
...
if [ $_ftype == "" ]; then
    ...
    if [ -f "$PWD/BBK/$_ftype.bbk" ]; then
        ...
        eval "./BBK/$_ftype.bbk -t wskey_convert_v2"
    else
        ...
        eval "$PWD/$_ftype.bbk -t wskey_convert_v2"
    fi
fi